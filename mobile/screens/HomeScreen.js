...
export default function HomeScreen({ route, navigation }) {
  ...
  const locationLabel = pickedLocation?.label || 'Pick exact location';
  ...
  return (
    <View style={styles.background}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.flex}>
        <ScrollView
          contentContainerStyle={[styles.content, { paddingTop: insets.top + 118, paddingBottom: insets.bottom + 124 }]}
        ...
        >
          <LiquidGlass style={styles.heroGlass} contentStyle={styles.heroContent} strong radius={RADII.lg}>
            <View style={styles.topControls}>
              <View style={styles.cityWrap}>
                <TouchableOpacity style={styles.cityButton} onPress={() => setCityOpen(value => !value)} activeOpacity={0.84}>
                  ...
                  <LiquidGlass style={styles.cityMenu} contentStyle={styles.cityMenuInner} radius={18}>
                    {SUPPORTED_CITIES.map(city => (
                      ...
                      >
                        ...
                        {city === selectedCity ? <Ionicons name="checkmark" size={16} color="#FFFFFF" /> : null}
                      </TouchableOpacity>
                    ))}
                  </LiquidGlass>
                ) : null}
              ...
              <TouchableOpacity style={styles.locationButton} onPress={() => navigation.navigate('LocationPicker', { pickedLocation })} activeOpacity={0.84}>
            ...
            <Text style={styles.subhero}>Describe the job in English, Urdu, or Roman Urdu. City selection keeps ambiguous areas precise.</Text>
          ...
          <View style={styles.serviceGrid}>
            {featuredServices.map(service => (
              <TouchableOpacity key={service.id} style={styles.serviceCard} onPress={() => prefill(service.label)} activeOpacity={0.86}>
